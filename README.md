...
num = true; // error because true is of type 'boolean,' but num is of type 'byte'

num = true as byte; // this works
```
```
var strPtr: &byte;

strPtr = &! "hello"; // error because the right side is of type '&(5,0)': a pointer to exactly 5 bytes,
                     // but strPtr is of type &byte: a pointer to an arbitrary amount of bytes
...
function main(): void {
  ...
  // or
  print function2(); // this does the same thing, because function2 is in the same file as main
}
...
## Terminology
### Compilation Unit
"Compilation unit" is used interchangeably with "source code file."
### Compiled Bytecode File
A "compiled bytecode file" is a file generated by the compiler as a result of running the `compile` command on a source code file. Bytecode files have the extension `.mnrc`.