    ...
    public static class Analysis extends Structure.Analysis implements Expr {
        ...
        protected Analysis(Raw raw, AnalysisContext ctx) {
            ...
            if (raw.type.analyze(ctx) instanceof TypeGet.Analysis typeGet) {
                if (typeGet.type() != null) {
                    if (isFloat(typeGet.type())) type = typeGet.type();
                    else ctx.report(new WrongTypeErr("target type", "float type"), this);
        ...
        public void assemble(AssembleContext ctx) {
            value.assemble(ctx);
            ctx.data().opIntToFloat(value.footprint(), type.footprint());
